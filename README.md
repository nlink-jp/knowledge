...
| [shell-scripting](docs/en/shell-scripting.md) | BSD/GNU sed differences, zsh expansion quirks, Bash trap scope, substitution pitfalls, empty arrays under set -u on bash 3.2, `$?` after an `if` statement, `! grep` reading an error as no match, telling an unfetched input from an empty answer, `git rev-parse` without `--verify`, xargs on empty input |
