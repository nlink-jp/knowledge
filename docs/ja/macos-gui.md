...
## `CFPreferencesSynchronize` は保存されなかった書き込みにも true を返す —— ファイルを確かめる

**2026-09-21 訂正。** このエントリは当初「`CFPreferencesSynchronize` が失敗した後、そのプロセスは自分の
読み取りを信じられない」と題し、2 状態の記録と「止まるプロセス」を処方していた。それは誰も測っていない
失敗に対する設計で、実測によって不要になった。

**実測（macOS 27.0、plist を変更不可にした使い捨ての設定ドメイン）:**
`CFPreferencesSetMultiple` + `CFPreferencesSynchronize` は **true** を返した。その後
`CFPreferencesCopyValue` は——書いたプロセスでも*新しいプロセスでも*——要求された値を 15 秒から 1 分ほど
返し続け、その後は元の値を返した。どの時点でもエラーは無い。ディスク上の plist は、書き込みの 0 ms 後から
元の値を保持していた。保存*された*書き込みの後は 0 ms 後から新しい値を保持しており、書き込みがファイルを
新規作成した場合も同じだった。`defaults read` はディスクと一致した。

**これが生んだ症状:** 書き込みを CFPreferences 経由の読み返しで検証するアプリは、失敗すべきまさにその場面で
検証に合格した。新しい値を表示・記録したが Mac は元の値のままで、次の Undo はその元の値を「外部の変更」と
みなして拒否した。

**どう適用するか:**
- 設定の書き込みが*保存されたか*を知るには plist を解析する（グローバルの current-host ドメインは
  `~/Library/Preferences/ByHost/.GlobalPreferences.<host uuid>.plist`、host uuid は `gethostuuid`）。
  `Synchronize` の戻り値も API 経由の読み返しも、別の問いに答えている。API は、ファイルが解析できない
  ときの代替として残し、実機テストで「ファイルが実際の書き込みを即座に示すこと」を確かめる——
  ファイルの場所は長年の挙動であって API ではない。
- 読み返しが真実を返すなら、「書く → 読み返す → 比べる → 違えばそう伝える」が設計のすべてで、戻り道の
  記録は後から事実に合わせるだけでよい: **Mac が変わっていなければ記録も変えない。そうでなければ Mac が
  持つ値を記録する。**
- **失敗に備えて設計する前に、その失敗を測る。** 設計を 3 つ書き、レビューを受け、ブロックされた
  （失敗後の読み取りで記録を確定・2 状態を名指す記録・続行を拒否するプロセス）。どれも直前のレビュー指摘
  への回答だった。3 つとも「失敗は*報告される*」を前提にしていた。20 行のプローブ 1 本で、報告されない
  ことが分かり、そのコードは一度も動き得なかった。修正のたびにレビューが同じクラスの欠陥を見つけるなら、
  パッチを止めて前提を検証する。
