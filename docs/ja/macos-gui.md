...
### 操作部品を持つポップオーバーは、開くときにアプリをアクティブ化する
...
**適用方法:**
- 操作部品のあるパネルでは、`show(relativeTo:)` と `makeKey()` の直後に `NSApp.activate` を呼ぶ。
  実測で表示の 12〜39 ms 後にアクティブ化が完了し（9 回中 9 回）、最初のクリックより前に済む。
  修正後、最短のメニューでも 909 ms 開いていた。
- 閉じたときに**自分がまだアクティブなら**、直前に最前面だったアプリへ返す
  （開く前に `NSWorkspace.shared.frontmostApplication` を覚え、`yieldActivation(to:)` +
  `activate()`）。ウィンドウを持たないアクセサリアプリがアクティブのまま残ると、利用者の
  キー入力の行き先が無くなる。外側クリックで別アプリが最前面になった場合は何もしない。
- 表示だけのパネル（操作部品なし）には不要。`makeKey()` だけで描画は同じになる。
