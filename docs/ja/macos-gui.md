...
## `CFPreferencesSynchronize` が失敗した後、そのプロセスは自分の読み取りを信じられない

**症状:** グローバル設定 2 つを書くアプリが「戻り道」の記録を持つ。最初の変更より前に*目標値*で保存し、
書き込み後の読み返しで訂正する。flush が失敗するとこの訂正が飛ばされ、記録は Mac が到達しなかった状態を
名指し、次の Undo は利用者自身の以前の変更を「外部の変更」として拒否した。修正は 2 つ書かれ、リリース前の
レビューで撤回され、3 つ目で持ちこたえた。(1) 失敗の後の読み取りで記録を確定させる: flush が失敗した
時点で `CFPreferencesSetMultiple` は既にプロセス自身の見え方を変えているので、その読み取りはディスクに
届かなかった値を返し得る —— 失敗した Undo が original を読み返し、記録は捨てられ、再起動後はアプリの値が
効いたまま戻り道が無くなった。(2) 記録には触らないが動作は続ける: *次のクリック*が同じ読み取りをして
それを信じた —— やり直した Undo は「すでに元どおり」と報告して記録を捨て、やり直した「既定に戻す」は、
次のプロセスにアプリ自身の値をその Mac の original として記録させた。失敗時のメッセージは
「もう一度試して」だった。

**どう適用するか:**
- 書き込み前の記録は、Mac があり得る**両方の**状態を名指す: 目標値と、置き換えられるのがアプリ自身の
  以前の値であるとき（外部の値では決してなく）その値。throw した書き込みは記録をそのまま残し、
  「この状態は自分のものか」の判定はどちらも受け入れる。
- flush が一度失敗したら**プロセス**に印を付ける: 判断のための読み取りと書き込みを以後は拒否し、
  ウィンドウの更新を止め（表示も読み取りである）、終了して開き直すよう伝える。新しいプロセスは
  ディスクから読み、2 状態の記録を、見つけた方に確定させる。
- テストでは、再起動は同じスタブの上の**新しい** coordinator であり、スタブの失敗は throw する前に
  *着地*できなければならない。coordinator を使い回すテストや、「throw して何も変わらない」しか知らない
  スタブは、上のすべてを素通りした。
- macOS が保存しなかった値をいつまで報告し続けるかは測っていない。その答えに何も依存しないよう設計する。
