...
## 仮想マシンには製品が実際に使うデバイスだけを与える — 余分な表示ヘッドがハイパーバイザを落とした

**何が起きたか:** テスト用ゲストに `-device virtio-gpu-pci,max_outputs=2` を与えていた。2 ヘッドの
レイアウトを検証する upstream のハーネスからの引き写しである。ゲストで SPICE エージェントが動き、
クライアントが接続と切断を繰り返す状況で、QEMU が core を吐いた（exit 139）。`qemu-system-arm`
8.2.2（Ubuntu 24.04）と 10.0.13（Debian 13）の両方で再現したので、更新は答えではなかった。試験対象の
アプリは単一の表示ストリームしか提示せず、2 本目のヘッドを要求したことは一度も無い。
`max_outputs=1` にしたところ、連続 11 回ピアは生存した（spice-client、2026-09-18）。

**適用方法:** フィクスチャのデバイス一覧は、製品自身の使い方の範囲までしか引き写さない。余分な
デバイスはすべて、製品が使わないのにテストだけが死にうる面である。ハイパーバイザがエラーではなく
クラッシュしたときは、ゲストのログより先に終了コードを見る。`--rm` で起動したコンテナはその終了
コードを捨てるので、フィクスチャのコンテナは持ち主が消すまで残すべきである。

**製品がその不安定なデバイスを必要とする場合は、外すのではなく隔離する。** 同じフィクスチャに後日
音声デバイスを足した。再生は実在の機能だからである。するとクラッシュが戻った。デバイスありで
12 回中 3 回、無しで 18 回中 0 回、新しいハイパーバイザでも 5 回中 2 回なので、更新は答えでは
なかった。クラッシュに必要だったのは反復である。再生チャンネルを開いたままクライアントが何度も
接続と切断を繰り返すことだった。実行を 2 フェーズに割り、反復の多いスイートはデバイス無しの
フィクスチャへ、音声の確認は専用フィクスチャで 1 接続だけにしたところ、5 回中 5 回、続いて全体
4 回中 4 回クリーンだった。直ったと宣言する前に前後の発生率を測ること。どちらも間欠であり、
1 回のクリーンな実行は何も証明しない。

音声固有の細部をもう 1 つ。無音のストリームでも経路全体を通せる。トランスポートは渡された PCM を
中身に関わらず運ぶので、ゲストがゼロを流してもクライアントのパケットカウンタは増え、ゲートを
走らせるマシンは静かなままである。音声の検証が、ノート PC から突然テストトーンが鳴ることを
意味してはならない。
