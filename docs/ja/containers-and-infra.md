...
## 一時ポートの「空きを探してから束縛する」は本質的に競合する — 直し方は有界な再試行

**事象:** コンテナに一時ポートを割り当てて公開する統合テストのゲートが、連続 3 回の
実行のうち 1 回落ちた。
...
フェーズ 1 のピアを停止した直後にフェーズ 2 のピアを起動する箇所で起きた。同じ手順が
その前後で何度も通っているので、設定の誤りではない。

**なぜ:** 一時ポートの公開は、カーネルに空きポートを尋ねてから実際に束縛するまでに
必ず隙間がある。その間は誰もそのポートを予約していない。直前に閉じたソケットが
TIME_WAIT を抜けた瞬間や、無関係なプロセスの接続が、同じ番号を先に取れてしまう。
コンテナランタイムの側にも、この隙間を無くす選択肢は無い（ポート番号を固定すれば
隙間は消えるが、今度は前の実行の残骸と衝突しうるので別の問題になる）。つまりこれは
実装の不具合ではなく、API の形に由来する競合である。頻度が低いので、ゲートを
「たまに落ちるもの」として扱う習慣が育ちやすい。

**適用方法:** 起動そのものを有界に再試行する。毎回、新しい割り当てを取り直すこと。
同じポートを待つのは意味が無い。

- 再試行の条件は、**エラー文字列で「アドレス使用中」だけに絞る**。それ以外の失敗
  （イメージが無い、引数が誤っている）は、そのままの終了状態と文言で即座に表に出す。
  全失敗を再試行すると、本当の故障が「5 回試して駄目でした」に化けて原因が消える。
- 再試行したことを標準エラーに 1 行残す。黙って直ると、頻度が上がったことに
  気付けなくなる。
- 再試行の前に、名前を持つコンテナの残骸を消す。失敗した起動が名前だけ取っている
  ことがある。
- 上限は小さくてよい（5 回で足りた）。上限に達したら、最後のエラー文をそのまま含めて
  失敗させる。

## `podman ps --filter name=` はアンカー無しの正規表現である

**症状:** あるサーバーがコンテナを `--filter name=tool-<id>` で引いていた。podman は `tool-gamma` を
`tool-gamma2-…` にも、別の作業ディレクトリで作った同じ id にも一致させた（podman 6.1.2 で実測:
使い捨てコンテナ 2 個で、素の名前は両方を、アンカー付きは 1 個を返す）。誤った一致が 1 件なら delete が
他人のコンテナを強制削除し、複数なら「ID」が 2 行で返って `podman rm` がそれで失敗した。

**どう適用するか:** 必ず `name=^<正規表現エスケープした名前>$` とし、1 つの関数で組み立てる。すべての
`ps` に同じ決め打ちの id を返す単体テスト用フェイクは、名前の誤りをそもそも見られない —— フェイクに
コンテナの表を持たせ、podman と同じ一致のしかた（アンカー無しの正規表現）をさせれば、アンカーの欠落が
単体テストで落ちる。同じ問いを実物の podman に尋ねる opt-in のテストも 1 本残す。
